...
export default function AssetFormPage() {
  ...
  const pageTitle = isEdit
    ? activeSubstansi
      ? `Edit Data ${visibleSections[0]?.label || "Aset"}`
      : "Edit Data Aset"
    : "Tambah Data Aset";
    ...
    <div className="min-h-full bg-surface-secondary p-4 md:p-6">
      <div className="mx-auto max-w-[1600px] space-y-5">
        <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-1.5 text-xs font-semibold text-text-muted">
          ...
          <span className="text-text-primary">{isEdit ? "Edit" : "Tambah"}</span>
        </nav>
          ...
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div className="flex items-start gap-4">
              <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-accent/10 text-accent">
                <ClipboardTextIcon size={22} weight="duotone" />
              </span>
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.14em] text-accent">
                  {isEdit ? asset?.kode_aset : "Registrasi aset baru"}
                </p>
                <h1 className="mt-1 text-xl font-black text-text-primary md:text-2xl">{pageTitle}</h1>
                <p className="mt-1 max-w-3xl text-xs leading-relaxed text-text-muted">
                  {isEdit
                    ? `Perbarui informasi ${asset?.nama_aset || "aset"} dan simpan perubahan sebelum kembali ke daftar.`
                    : "Lengkapi data aset secara bertahap. Kolom wajib ditandai pada setiap bagian form."}
                </p>
              </div>
            </div>
            <button
              type="button"
              onClick={() => navigate(returnPath)}
              className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-border bg-surface px-4 text-xs font-bold text-text-secondary transition hover:border-accent hover:text-accent focus-visible:ring-2 focus-visible:ring-accent"
            >
        ...
        <div className="sr-only" aria-live="polite">
          {isEdit
            ? `Mengedit aset ${asset?.kode_aset || id}`
